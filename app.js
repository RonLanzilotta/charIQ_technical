...
async function fetchStockData() {
    try {
        ...
        // Object.entries() returns an array of arrays which contain two elements each, derived from the stockData object's key value pairs.
        // Here, .map() returns an array of objects with the date and the stock's closing price can be found by accessing line 4 (close) of the raw data.
        const stockPrices = Object.entries(stockData).map(([date, values]) => {
            return { date, price: parseFloat(values['4. close'])};
        })
...
function drawChart(data) {
    // Create arrays to store the dates and prices in separate arrays for mapping and dom manipulation
